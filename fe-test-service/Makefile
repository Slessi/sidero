...
all: unit-tests server image-server lint
...
.PHONY: $(ARTIFACTS)/server-darwin-amd64
$(ARTIFACTS)/server-darwin-amd64:
	@$(MAKE) local-server-darwin-amd64 DEST=$(ARTIFACTS)

.PHONY: server-darwin-amd64
server-darwin-amd64: $(ARTIFACTS)/server-darwin-amd64  ## Builds executable for server-darwin-amd64.

.PHONY: $(ARTIFACTS)/server-darwin-arm64
$(ARTIFACTS)/server-darwin-arm64:
	@$(MAKE) local-server-darwin-arm64 DEST=$(ARTIFACTS)

.PHONY: server-darwin-arm64
server-darwin-arm64: $(ARTIFACTS)/server-darwin-arm64  ## Builds executable for server-darwin-arm64.

.PHONY: $(ARTIFACTS)/server-linux-amd64
$(ARTIFACTS)/server-linux-amd64:
	@$(MAKE) local-server-linux-amd64 DEST=$(ARTIFACTS)

.PHONY: server-linux-amd64
server-linux-amd64: $(ARTIFACTS)/server-linux-amd64  ## Builds executable for server-linux-amd64.

.PHONY: $(ARTIFACTS)/server-linux-arm64
$(ARTIFACTS)/server-linux-arm64:
	@$(MAKE) local-server-linux-arm64 DEST=$(ARTIFACTS)

.PHONY: server-linux-arm64
server-linux-arm64: $(ARTIFACTS)/server-linux-arm64  ## Builds executable for server-linux-arm64.

.PHONY: server
server: server-darwin-amd64 server-darwin-arm64 server-linux-amd64 server-linux-arm64  ## Builds executables for server.

.PHONY: lint
lint: lint-golangci-lint lint-gofumpt lint-govulncheck  ## Run all linters for the project.

.PHONY: image-server
image-server:  ## Builds image for server.
	@$(MAKE) registry-$@ IMAGE_NAME="server"
